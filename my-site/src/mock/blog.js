...
// 博客文章的模拟数据
Mock.mock(/^\/api\/blog(\?.+)?$/, 'get', function (search) {
    const limit = search.url.split('?')[1].split('&');
    const searchObj = {};
    for (let i = 0; i < limit.length; i++) {
        const key = limit[i].split('=')[0];
        const value = limit[i].split('=')[1];
        searchObj[key] = Number(value);
    }
    return Mock.mock({
        "code": 0, // 错误码
        "msg": "", // 错误消息
            ...
            [`rows|${searchObj.limit || 10}`]: [
                {
                    ...
                    "scanNumber|500-3000": 10,//查看数量
                    "commentNumber|0-3000": 30,//评论数量
                    thumb: "@image('250x200', '@color', '@color', 'xiaochen')",
                    createDate: "@date('T')"
                }
            ]
        }
...
// 单条博客的模拟数据
Mock.mock(/^\/api\/blog\/[^/]+$/, 'get', function (url) {
...
// 提交评论
Mock.mock("/api/comment", "post", function (commentInfo) {
    const comment = JSON.parse(commentInfo.body);
    return Mock.mock({
        code: 0,
        msg: "",
        data: {
...
// 获取评论
Mock.mock(/\/api\/comment(\?.+)?$/, 'get', function (search) {
    const blogLimit = search.url.split('?')[1].split('&');
    const commentObj = {};
    for (let i = 0; i < blogLimit.length; i++) {
        const key = blogLimit[i].split('=')[0];
        const value = blogLimit[i].split('=')[1];
        commentObj[key] = Number(value);
    }
    return Mock.mock({
        code: 0,
        msg: "",
